...
import torch
from model import VAB, xattn_score_t2i, xattn_score_i2t
...
def encode_data(model, data_loader, log_step=10, logging=print):
    ...
    for i, (images, captions, lengths, ids) in enumerate(data_loader):
        ...
        if img_embs is None:
            ...
            cap_embs = np.zeros((len(data_loader.dataset), cap_emb.size(1)))    #维度是batch*dim就用这个
            cap_lens = [0] * len(data_loader.dataset)

        # cache embeddings
        img_embs[ids] = img_emb.data.cpu().numpy().copy()
        #cap_embs[ids,:max(lengths),:] = cap_emb.data.cpu().numpy().copy()
        cap_embs[ids] = cap_emb.data.cpu().numpy().copy()          #维度是batch*dim就用这个
...
def shard_xattn_t2i(images, captions, caplens, opt, shard_size=128):
    ...
    for i in range(int(n_im_shard)):
        ...
        for j in range(int(n_cap_shard)):
            ...
            sim = F.normalize(im.mean(dim=1)) @ F.normalize(s.mean(dim=1))
