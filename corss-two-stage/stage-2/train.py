...
from evaluation import shard_xattn_i2t,shard_xattn_t2i,scan_i2t,scan_t2i,dire_xattn

def main():
    # Hyper Parameters
    parser = argparse.ArgumentParser()
    parser.add_argument('--logger_name', default='runs/runX', help='Path to save the model and Tensorboard log.')
    parser.add_argument('--data_path', default='/data2/liaojunrong', help='path to datasets')
    parser.add_argument('--data_name', default='f30k_precomp', help='{coco,f8k,f30k,10crop}_precomp|coco|f8k|f30k')
    parser.add_argument("--bert_path", default='/data2/liaojunrong/uncased_L-12_H-768_A-12', type=str, help="The BERT model path.")
    ...
    parser.add_argument('--val_step', default=50, type=int, help='Number of steps to run validation.')
    ...
    writer = SummaryWriter('board/orignal')
    
    torch.manual_seed(15070824111340987151)
    torch.cuda.manual_seed_all(271674164741741)


    logging.info(torch.initial_seed())
    logging.info(torch.cuda.initial_seed())
    ...
    for epoch in range(opt.num_epochs):
        ...
        # train for one epoch
        best_rsum, best_r1 = train(opt, train_loader, model, epoch, val_loader, best_rsum, best_r1, writer)
...
def train(opt, train_loader, model, epoch, val_loader, best_rsum, best_r1, writer):
    ...
    for i, train_data in enumerate(train_loader):
        ...
        # Update the model
        loss = model.train_emb(epoch, train_data, writer)
...
def validate(opt, val_loader, model):
    ...
    #计算SCAN的recalls
    img_embs, cap_embs, cap_lens = embs[0], embs[2], embs[4]
    img_embs = numpy.array([img_embs[i] for i in range(0, len(img_embs), 5)])

    start = time.time()
    '''
    if opt.direction == 'i2t':    #注意这里也要反过来
        sims = shard_xattn_t2i(img_embs, cap_embs, cap_lens, opt, shard_size=128)
    elif opt.direction == 't2i':
        sims = shard_xattn_i2t(img_embs, cap_embs, cap_lens, opt, shard_size=128)
    else:
        raise NotImplementedError
    '''

    sims = dire_xattn(img_embs,cap_embs,cap_lens,opt,shard_size=128)
    end = time.time()
    print("calculate similarity time:", end - start)

    # caption retrieval
    (sr1, sr5, sr10,sr25,sr50,sr75,sr100, smedr, smeanr) = scan_i2t(img_embs, cap_embs, cap_lens, sims)
    logging.info("Image to text: %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f" %
                 (sr1, sr5, sr10,sr25,sr50,sr75,sr100, smedr, smeanr))
    # image retrieval
    (sr1i, sr5i, sr10i,sr25i,sr50i,sr75i,sr100i, smedri, smeanr) = scan_t2i(
        img_embs, cap_embs, cap_lens, sims)
    logging.info("Text to image: %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f, %.1f" %
                 (sr1i, sr5i, sr10i,sr25i,sr50i,sr75i,sr100i, smedri, smeanr))
