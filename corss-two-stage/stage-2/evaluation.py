...
from models.SCAN import xattn_score_t2i, xattn_score_i2t, calscores
...
def encode_data(model, data_loader, opt, log_step=10, logging=print):
    ...
    for i, batch_data in enumerate(data_loader):
        ...
        images, boxes, img_whs, input_ids, lengths, ids, attention_mask, token_type_ids = batch_data
...
def dire_xattn(images, captions, caplens, opt, shard_size=128):
    """
        Computer pairwise i2t image-caption distance with locality sharding
    """
    n_im_shard = (len(images) - 1) / shard_size + 1
...
def scan_i2t(images, captions, caplens, sims, npts=None, return_ranks=False):
    """
    ...
    """
    sims_new = np.zeros_like(sims)

    npts = images.shape[0]
    ranks = np.zeros(npts)
    top1 = np.zeros(npts)
    for index in range(npts):
        inds = np.argsort(sims[index])[::-1]  # 从大到小排列，输出索引值
        sims_new[index] = inds

        # Score
        rank = 1e20
        for i in range(5 * index, 5 * index + 5, 1):
            tmp = np.where(inds == i)[0][0]  # tmp就是ids中等于i的那个元素的索引
            if tmp < rank:
                rank = tmp
        ranks[index] = rank  #
        top1[index] = inds[0]

    sims_new = sims_new.astype(int)
    #np.save("sim_matrix.npy", sims_new)
    ...
    r75 = 100.0 * len(np.where(ranks < 75)[0]) / len(ranks)
    r100 = 100.0 * len(np.where(ranks < 100)[0]) / len(ranks)
    medr = np.floor(np.median(ranks)) + 1
    meanr = ranks.mean() + 1
    if return_ranks:
        return (r1, r5, r10, r25, r50, r75, r100, medr, meanr), (ranks, top1)
    else:
        return (r1, r5, r10, r25, r50, r75, r100, medr, meanr)


def scan_t2i(images, captions, caplens, sims, npts=None, return_ranks=False):
    """
