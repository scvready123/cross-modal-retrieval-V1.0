    ...
    def __init__(self, data_path, data_split, opt):
        ...
        with open(loc+'%s_caps.txt' % data_split, 'rb') as f:
            for line in f:
                ...
                tokens = nltk.tokenize.word_tokenize(str(line.strip()).lower())  # python2
...
def get_loaders(data_name, batch_size, workers, opt):
    ...
    if opt.data_name.endswith('_precomp'):
        ...
        val_loader = get_precomp_loader(dpath, 'test',  opt, batch_size, False, workers)  #
    else:
        # Build Dataset Loader
        train_loader, val_loader = get_precomp_loader(dpath, 'train', opt, batch_size, True,workers), get_precomp_loader(dpath, 'train', opt, batch_size,True, workers)
