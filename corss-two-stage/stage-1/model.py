...
class DIME(object):
    def __init__(self, opt):
        ...
        self.img_enc = EncoderImage(opt, opt.data_name, opt.img_dim, opt.embed_size, opt.direction, 
        ...
        # Loss and Optimizer
        self.criterion = ContrastiveLoss(opt, margin=opt.margin, max_violation=opt.max_violation)
    ...
    def itr_module(self, rgn, wrd):

        sims = torch.matmul(rgn, wrd.t())
    ...
    def train_emb(self, epoch, batch_data, *args):
        ...
        sim_mat = self.itr_module(self_att_emb, word_emb)
        #sim_rgn = self.itr_module(self_att_emb, self_att_emb)
        #sim_wrd = self.itr_module(word_emb, word_emb)
