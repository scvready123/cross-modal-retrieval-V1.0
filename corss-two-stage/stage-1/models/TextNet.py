...
class EncoderText(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, token_type_ids, lengths):
        ...
        #print('all_encoder_layers[-1]',all_encoder_layers[-1].size())
        #print('pooled_output',len(pooled_output),len(pooled_output[0]))
        #这里很奇怪，all_encoder_layers[-1]维度是 batch*768,所以应该改成0？

        #x = all_encoder_layers[-1].unsqueeze(1)
        if self.training:
            bert_emb = all_encoder_layers[0].detach().mean(dim=1)
            bert_emb = F.normalize(bert_emb, dim=-1)
        x_emb = self.fc(all_encoder_layers[0])
        ...
        txt_emb = x_emb.mean(1)
        txt_emb = F.normalize(txt_emb, p=2, dim=-1)
        word_emb = F.normalize(word_emb, p=2, dim=-1)

        
        word_emb = self.sa(word_emb)

        n_stc, n_wrd, d = word_emb.size()
        word_emb = (self.bn(word_emb.view(n_stc*n_wrd, d))).view(n_stc, n_wrd, d)
        word_emb = word_emb.mean(1) + word_emb.max(1)[0]
