...
def encode_data(model, data_loader, opt, log_step=10, logging=print):
    ...
    for i, batch_data in enumerate(data_loader):
        ...
        #print('img_emb',img_emb.size()) #128,256
        #print('self_att_emb', self_att_emb.size()) #128,256
        #print('cap_emb', cap_emb.size()) #128
        #print('word_emb', word_emb.size()) #128,256

        # initialize the numpy arrays given the size of the embeddings
        if is_init:
            is_init = False
            rgn = np.zeros((len(data_loader.dataset), self_att_emb.size(1)), dtype=np.float32)
            img = np.zeros((len(data_loader.dataset), img_emb.size(1)), dtype=np.float32)
            wrd = np.zeros((len(data_loader.dataset), word_emb.size(1)), dtype=np.float32)
    ...
    for i in range(n_im_shard):
        ...
        for j in range(n_cap_shard):
            ...
            with torch.no_grad():
                ...
                sim = model.itr_module(rgn_block, wrd_block)
