...
def main():
    ...
    parser.add_argument('--data_path', default='/data1/lihaoxuan/datasets/all/data/', help='path to datasets')
    parser.add_argument('--data_name', default='f30k_precomp', help='{coco,f8k,f30k,10crop}_precomp|coco|f8k|f30k')
    parser.add_argument("--bert_path", default='/data1/lihaoxuan/uncased_L-12_H-768_A-12/', type=str, help="The BERT model path.")
    ...
    model = DIME(opt)
    model_file = '/data2/lihaoxuan/DIME/two-stage-1-triplet/runs/flickr_DIME/model_best.pth.tar'
    ...
    for i, batch_data in enumerate(train_loader):
        ...
        if is_init:
            is_init = False
            rgn = np.zeros((len(train_loader.dataset), self_att_emb.size(1)), dtype=np.float32)
            #img = np.zeros((len(train_loader.dataset), img_emb.size(1)), dtype=np.float32)
            wrd = np.zeros((len(train_loader.dataset), word_emb.size(1)), dtype=np.float32)
            #stc = np.zeros((len(train_loader.dataset), cap_emb.size(1)), dtype=np.float32)
            #stc_lens = np.zeros(len(train_loader.dataset), dtype=np.int)

        rgn[ids] = self_att_emb.detach().cpu().numpy().copy()
        #img[ids] = img_emb.data.cpu().numpy().copy()
       # cur_max_len = word_emb.size(1)
        wrd[ids, :] = word_emb.data.cpu().numpy().copy()
        ...
        del batch_data
    rgn = np.array([rgn[i] for i in range(0, len(rgn), 5)])
    np.save('/data2/lihaoxuan/DIME/two-stage-1-triplet/runs/image.npy', rgn)
    np.save('/data2/lihaoxuan/DIME/two-stage-1-triplet/runs/text.npy', wrd)
